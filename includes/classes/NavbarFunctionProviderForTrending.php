<?php
class NavbarFunctionProviderForTrending{
    ...
    public function generateItemsComedy($categ) {
        $query = $this->con->prepare("SELECT * FROM videos GROUP bY category HAVING views = ( SELECT MAX(views) FROM videos GROUP BY category)");
        $query->execute();
